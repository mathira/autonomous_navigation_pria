...
from autonomous_navigation_pria.control import NavigationConfig
...
def test_goal_launch_arguments_default_to_a_free_pose_near_the_green_block(
    ...
    assert "".join(item.perform(LaunchContext()) for item in declaration.default_value) == expected


def test_launch_and_control_defaults_match_design():
    description = load_launch_description()
    context = LaunchContext()
    for declaration in launch_arguments(description).values():
